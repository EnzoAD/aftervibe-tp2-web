...
$sql = $pdo->prepare("update ingresso set nome = :nome , preco = :preco , descricao = :descricao where id = :id ");
$sql->bindValue(':id', $_POST['idingresso']);
$sql->bindValue(':nome', $_POST['nome']);
$sql->bindValue(':preco', $_POST['preco']);
$sql->bindValue(':descricao', $_POST['descricao']);
$sql->execute();

$sql2 = $pdo->prepare("delete from ingresso_programacao where id_ingresso = :id");
$sql2->bindValue(':id', $_POST['idingresso']);
$sql2->execute();
...
if ($sql2->rowCount() > 0){
    ...
    foreach ($events as $event) {
        if (!empty($_POST['op'.$i])){
            ...
            $sql3->bindValue(':id_ingresso', $_POST['idingresso']);
            $sql3->execute();
        }
        $i++;
    }
